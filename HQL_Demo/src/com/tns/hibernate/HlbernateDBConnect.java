...
public class HlbernateDBConnect 
{
	private static SessionFactory objSF;
	
	public static SessionFactory getSessionFactory()//it returns session factory objects
	{
		if(objSF==null) 
		{
		...
		try
	{
		...
		//its collectionclass where the data is stored in key value pairs
		objprop.put(Environment.DRIVER,"com.mysql.cj.jdbc.Driver");
		objprop.put(Environment.URL,"jdbc:mysql://localhost:3307/b7_hibernate_db");
		objprop.put(Environment.USER,"root");
		objprop.put(Environment.PASS,"password");
		
		objprop.put(Environment.DIALECT,"org.hibernate.dialect.MySQL5Dialect");//says dial to the connection
